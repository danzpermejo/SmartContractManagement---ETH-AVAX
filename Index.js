...
const App = () => {
  ...
  const setItemPrice = () => {
    ...
    const itemMaxPrice = parseFloat(maxPrice);
    if (isNaN(itemPrice) || itemPrice <= 0) {
      setMessage("Error: Price must be a number greater than zero");
      return;
    }
    if (!isNaN(itemMaxPrice) && itemPrice > itemMaxPrice) {
      setMessage(`Error: Price exceeds maximum price of ${maxPrice} ETH`);
      return;
    }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: 'Arial, sans-serif', background: 'transparent' }}>
      <div style={{ maxWidth: '600px', width: '100%', margin: '0 auto', padding: '20px', border: '1px solid #ccc', borderRadius: '5px', backgroundColor: 'rgba(255, 255, 255, 0.9)' }}>
        <h1 style={{ color: '#333', textAlign: 'center' }}>Crypto Price Tracker</h1>
        <div style={{ marginBottom: '10px' }}>
          ...
          <input 
            ...
            style={{ padding: '8px', marginRight: '10px', borderRadius: '5px', border: '1px solid #ccc', fontSize: '16px', width: 'calc(25% - 10px)' }}
          />
          <input 
            type="text" 
            ...
            style={{ padding: '8px', borderRadius: '5px', border: '1px solid #ccc', fontSize: '16px', width: 'calc(25% - 10px)' }}
          />
        </div>
        <div style={{ marginBottom: '10px' }}>
          <button 
